<?php
/**
 * Top navigation header for the Maccabipedia skin (SkinMustache-based).
 *
 * Identical-output port of skins/Metrolook/customize/includes/app-header.php
 * adapted for SkinMustache: receives `$skin` and `$searchBoxHtml` from
 * SkinMaccabipedia::getTemplateData() instead of relying on `$this->`-style
 * QuickTemplate methods. Once the Metrolook skin is removed in Phase 4,
 * the legacy copy under skins/Metrolook/customize/ goes away.
 *
 * Renders: logo + main-page link, four primary dropdown menus, the
 * "מכבימדיה" link block, and the right-side options panel
 * (edit/user/options dropdowns). The search box at the bottom is the
 * pre-rendered SkinMustache form passed in via $searchBoxHtml.
 */
...
<header class="app-header">
    <div class="content">
        ...
        <div class="search-container">
            <?php
            // SkinMustache equivalent of MetrolookTemplate's
            // $this->renderNavigation(['SEARCH']) — the search-box HTML is
            // pre-rendered by SkinMaccabipedia::getTemplateData() and passed
            // in as $searchBoxHtml so this template stays $this-free.
            echo $searchBoxHtml ?? '';
            ?>
        </div>
    </div>
