...
football_players_category_name = "קטגוריה:שחקנים"

NAME = "שם מלא"
DOES_MACCABI_PLAYER = "שחקן מכבי"
...
def get_players_to_add():
    ...
    all_players_names_without_dups = list(set([p.name for p in maccabi_games.available_players if p.name != ""]))
...
def generate_page_name_from_player_name(player_name):
    ...
    page_name = "{prefix}: {name}".format(prefix=player_prefix, name=player_name)
...
def __get_football_player_template(player_name):
    ...
    template_arguments[DOES_MACCABI_PLAYER] = "כן"
...
def create_or_update_player_page(player_name):
    ...
    page_name_without_namespace = pw.Page(site, page_name.replace(player_prefix, "").strip())
    save_this_page = True
    ...
    elif page_name_without_namespace.exists():
        save_this_page = False
        logger.info("Found page {name} without שחקן: in page name, this is probably opened without this bot, continue".format(name=player_name))
    else:
    ...
    if SHOULD_SAVE and save_this_page:
        ...
        player_page.save(summary="MaccabiBot - Add Players")
    else:
...
def main():
    ...
    players_names = get_players_to_add()
